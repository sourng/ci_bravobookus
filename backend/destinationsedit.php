...
class cdestinations_edit extends cdestinations {
	...
	function RenderRow() {
		...
		$this->country->ViewCustomAttributes = "";
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->dest_landmark);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// dest_landmark
			if ($this->dest_landmark->Visible && !$this->dest_landmark->ReadOnly && !$this->dest_landmark->Upload->KeepFile) {
				$this->dest_landmark->Upload->DbValue = $rsold['dest_landmark']; // Get original value
...
<?php if (@$_POST["fa_x_dest_landmark"] == "0") { ?>
<input type="hidden" name="fa_x_dest_landmark" id= "fa_x_dest_landmark" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_dest_landmark" id= "fa_x_dest_landmark" value="1">
<?php } ?>
