...
class ccountry_list extends ccountry {
	...
	function Page_Init() {
		...
		$this->country_id->Visible = !$this->IsAddOrEdit();
		$this->iso->SetVisibility();
		$this->printable_name->SetVisibility();
		$this->iso3->SetVisibility();
		$this->numcode->SetVisibility();
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->iso->setFormValue($arrKeyFlds[0]);
		}
		return TRUE;
	}
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server") {
			$sSavedFilterList = isset($UserProfile) ? $UserProfile->GetSearchFilters(CurrentUserName(), "fcountrylistsrch") : "";
		} else {
			$sSavedFilterList = "";
		}
		...
		$sFilterList = ew_Concat($sFilterList, $this->printable_name->AdvancedSearch->ToJSON(), ","); // Field printable_name
	...
	function RestoreFilterList() {
		...
		$this->printable_name->AdvancedSearch->SearchValue = @$filter["x_printable_name"];
		$this->printable_name->AdvancedSearch->SearchOperator = @$filter["z_printable_name"];
		$this->printable_name->AdvancedSearch->SearchCondition = @$filter["v_printable_name"];
		$this->printable_name->AdvancedSearch->SearchValue2 = @$filter["y_printable_name"];
		$this->printable_name->AdvancedSearch->SearchOperator2 = @$filter["w_printable_name"];
		$this->printable_name->AdvancedSearch->Save();
	...
	function BasicSearchSQL($arKeywords, $type) {
		...
		$this->BuildBasicSearchSQL($sWhere, $this->printable_name, $arKeywords, $type);
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->printable_name); // printable_name
	...
	function RenderListOptions() {
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" value=\"" . ew_HtmlEncode($this->iso->CurrentValue) . "\" onclick='ew_ClickMultiCheckbox(event);'>";
	...
	function LoadRowValues(&$rs) {
		...
		$this->printable_name->setDbValue($rs->fields('printable_name'));
	...
	function LoadDbValues(&$rs) {
		...
		$this->printable_name->DbValue = $row['printable_name'];
	...
	function LoadOldRecord() {
		...
		$bValidKey = TRUE;
		if (strval($this->getKey("iso")) <> "")
			$this->iso->CurrentValue = $this->getKey("iso"); // iso
		else
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		// printable_name
		$this->printable_name->ViewValue = $this->printable_name->CurrentValue;
		$this->printable_name->ViewCustomAttributes = "";
		...
		$this->numcode->ViewCustomAttributes = "";
			...
			// printable_name
			$this->printable_name->LinkCustomAttributes = "";
			$this->printable_name->HrefValue = "";
			$this->printable_name->TooltipValue = "";
...
<div class="panel panel-default ewGrid country">
<?php if ($country->Export == "") { ?>
...
<?php if ($country->country_id->Visible) { // country_id ?>
	<?php if ($country->SortUrl($country->country_id) == "") { ?>
		<th data-name="country_id"><div id="elh_country_country_id" class="country_country_id"><div class="ewTableHeaderCaption"><?php echo $country->country_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="country_id"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $country->SortUrl($country->country_id) ?>',1);"><div id="elh_country_country_id" class="country_country_id">
...
<?php if ($country->iso->Visible) { // iso ?>
	...
	<?php } ?>
<?php } ?>		
<?php if ($country->printable_name->Visible) { // printable_name ?>
	<?php if ($country->SortUrl($country->printable_name) == "") { ?>
		<th data-name="printable_name"><div id="elh_country_printable_name" class="country_printable_name"><div class="ewTableHeaderCaption"><?php echo $country->printable_name->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="printable_name"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $country->SortUrl($country->printable_name) ?>',1);"><div id="elh_country_printable_name" class="country_printable_name">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $country->printable_name->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($country->printable_name->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($country->printable_name->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
...
<?php echo $country->iso->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($country->printable_name->Visible) { // printable_name ?>
		<td data-name="printable_name"<?php echo $country->printable_name->CellAttributes() ?>>
<span id="el<?php echo $country_list->RowCnt ?>_country_printable_name" class="country_printable_name">
<span<?php echo $country->printable_name->ViewAttributes() ?>>
<?php echo $country->printable_name->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($country->iso3->Visible) { // iso3 ?>
