...
class ccountry extends cTable {
	...
	function __construct() {
		...
		// printable_name
		$this->printable_name = new cField('country', 'country', 'x_printable_name', 'printable_name', '`printable_name`', '`printable_name`', 200, -1, FALSE, '`printable_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->printable_name->Sortable = TRUE; // Allow sort
		$this->fields['printable_name'] = &$this->printable_name;

		// iso3
		$this->iso3 = new cField('country', 'country', 'x_iso3', 'iso3', '`iso3`', '`iso3`', 200, -1, FALSE, '`iso3`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->iso3->Sortable = TRUE; // Allow sort
		...
		$this->numcode->Sortable = TRUE; // Allow sort
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('iso', $rs))
				ew_AddFilter($where, ew_QuotedName('iso', $this->DBID) . '=' . ew_QuotedValue($rs['iso'], $this->iso->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@iso@", ew_AdjustSql($this->iso->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "iso:" . ew_VarToJson($this->iso->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->iso->CurrentValue)) {
			$sUrl .= "iso=" . urlencode($this->iso->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["iso"]))
				$arKeys[] = ew_StripSlashes($_POST["iso"]);
			elseif (isset($_GET["iso"]))
				$arKeys[] = ew_StripSlashes($_GET["iso"]);
			else
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->printable_name->EditValue = $this->printable_name->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->printable_name->Exportable) $Doc->ExportCaption($this->printable_name);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->printable_name->Exportable) $Doc->ExportField($this->printable_name);
