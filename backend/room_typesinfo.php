...
//
// Table class for room_types
//
class croom_types extends cTable {
	var $rt_id;
	var $rt_name;
	var $rt_image;
	var $rt_note;
	...
	function __construct() {
		...
		$this->TableVar = 'room_types';
		$this->TableName = 'room_types';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`room_types`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// rt_id
		$this->rt_id = new cField('room_types', 'room_types', 'x_rt_id', 'rt_id', '`rt_id`', '`rt_id`', 3, -1, FALSE, '`rt_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// rt_name
		$this->rt_name = new cField('room_types', 'room_types', 'x_rt_name', 'rt_name', '`rt_name`', '`rt_name`', 200, -1, FALSE, '`rt_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->rt_name->Sortable = TRUE; // Allow sort
		$this->fields['rt_name'] = &$this->rt_name;

		// rt_image
		$this->rt_image = new cField('room_types', 'room_types', 'x_rt_image', 'rt_image', '`rt_image`', '`rt_image`', 200, -1, FALSE, '`rt_image`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->rt_image->Sortable = TRUE; // Allow sort
		$this->fields['rt_image'] = &$this->rt_image;

		// rt_note
		$this->rt_note = new cField('room_types', 'room_types', 'x_rt_note', 'rt_note', '`rt_note`', '`rt_note`', 200, -1, FALSE, '`rt_note`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->rt_note->Sortable = TRUE; // Allow sort
		$this->fields['rt_note'] = &$this->rt_note;

		// rt_status
		$this->rt_status = new cField('room_types', 'room_types', 'x_rt_status', 'rt_status', '`rt_status`', '`rt_status`', 202, -1, FALSE, '`rt_status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'CHECKBOX');
		$this->rt_status->Sortable = TRUE; // Allow sort
		$this->rt_status->FldDataType = EW_DATATYPE_BOOLEAN;
		$this->rt_status->TrueValue = 'Y';
		$this->rt_status->FalseValue = 'N';
		$this->rt_status->OptionCount = 2;
		$this->fields['rt_status'] = &$this->rt_status;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`room_types`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->rt_id->setDbValue($conn->Insert_ID());
			$rs['rt_id'] = $this->rt_id->DbValue;
		}
		return $bInsert;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("room_typesview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("room_typesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "room_typesadd.php?" . $this->UrlParm($parm);
		else
			$url = "room_typesadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("room_typesedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("room_typesadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("room_typesdelete.php", $this->UrlParm());
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["rt_id"]))
				$arKeys[] = ew_StripSlashes($_POST["rt_id"]);
			elseif (isset($_GET["rt_id"]))
				$arKeys[] = ew_StripSlashes($_GET["rt_id"]);
			else
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->rt_name->EditValue = $this->rt_name->CurrentValue;
		...
		$this->rt_image->EditValue = $this->rt_image->CurrentValue;
		...
		$this->rt_note->EditValue = $this->rt_note->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->rt_name->Exportable) $Doc->ExportCaption($this->rt_name);
					if ($this->rt_image->Exportable) $Doc->ExportCaption($this->rt_image);
					if ($this->rt_note->Exportable) $Doc->ExportCaption($this->rt_note);
					if ($this->rt_status->Exportable) $Doc->ExportCaption($this->rt_status);
				} else {
					if ($this->rt_id->Exportable) $Doc->ExportCaption($this->rt_id);
					if ($this->rt_name->Exportable) $Doc->ExportCaption($this->rt_name);
					if ($this->rt_image->Exportable) $Doc->ExportCaption($this->rt_image);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->rt_name->Exportable) $Doc->ExportField($this->rt_name);
						if ($this->rt_image->Exportable) $Doc->ExportField($this->rt_image);
						if ($this->rt_note->Exportable) $Doc->ExportField($this->rt_note);
						if ($this->rt_status->Exportable) $Doc->ExportField($this->rt_status);
					} else {
						if ($this->rt_id->Exportable) $Doc->ExportField($this->rt_id);
						if ($this->rt_name->Exportable) $Doc->ExportField($this->rt_name);
						if ($this->rt_image->Exportable) $Doc->ExportField($this->rt_image);
