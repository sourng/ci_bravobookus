...
class cservices extends cTable {
	...
	function __construct() {
		...
		// service_id
		$this->service_id = new cField('services', 'services', 'x_service_id', 'service_id', '`service_id`', '`service_id`', 3, -1, FALSE, '`service_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// service_name
		$this->service_name = new cField('services', 'services', 'x_service_name', 'service_name', '`service_name`', '`service_name`', 200, -1, FALSE, '`service_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->service_name->Sortable = TRUE; // Allow sort
		$this->fields['service_name'] = &$this->service_name;

		// service_desc
		$this->service_desc = new cField('services', 'services', 'x_service_desc', 'service_desc', '`service_desc`', '`service_desc`', 200, -1, FALSE, '`service_desc`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->service_desc->Sortable = TRUE; // Allow sort
		$this->fields['service_desc'] = &$this->service_desc;

		// service_icon
		$this->service_icon = new cField('services', 'services', 'x_service_icon', 'service_icon', '`service_icon`', '`service_icon`', 200, -1, FALSE, '`service_icon`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->service_icon->Sortable = TRUE; // Allow sort
		$this->fields['service_icon'] = &$this->service_icon;

		// service_image
		$this->service_image = new cField('services', 'services', 'x_service_image', 'service_image', '`service_image`', '`service_image`', 200, -1, FALSE, '`service_image`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->service_image->Sortable = TRUE; // Allow sort
		$this->fields['service_image'] = &$this->service_image;

		// service_link
		$this->service_link = new cField('services', 'services', 'x_service_link', 'service_link', '`service_link`', '`service_link`', 200, -1, FALSE, '`service_link`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->service_link->Sortable = TRUE; // Allow sort
		$this->fields['service_link'] = &$this->service_link;

		// status
		$this->status = new cField('services', 'services', 'x_status', 'status', '`status`', '`status`', 3, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["service_id"]))
				$arKeys[] = ew_StripSlashes($_POST["service_id"]);
			elseif (isset($_GET["service_id"]))
				$arKeys[] = ew_StripSlashes($_GET["service_id"]);
			else
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->service_icon->EditValue = $this->service_icon->CurrentValue;
		...
		$this->service_image->EditValue = $this->service_image->CurrentValue;
		...
		$this->service_link->EditValue = $this->service_link->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->service_desc->Exportable) $Doc->ExportCaption($this->service_desc);
					if ($this->service_icon->Exportable) $Doc->ExportCaption($this->service_icon);
					if ($this->service_image->Exportable) $Doc->ExportCaption($this->service_image);
					if ($this->service_link->Exportable) $Doc->ExportCaption($this->service_link);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->service_desc->Exportable) $Doc->ExportField($this->service_desc);
						if ($this->service_icon->Exportable) $Doc->ExportField($this->service_icon);
						if ($this->service_image->Exportable) $Doc->ExportField($this->service_image);
						if ($this->service_link->Exportable) $Doc->ExportField($this->service_link);
