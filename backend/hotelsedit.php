...
class chotels_edit extends chotels {
	...
	function RenderRow() {
		...
		$this->hotel_blocked->ViewCustomAttributes = "";
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->h_feature_image);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// h_feature_image
			if ($this->h_feature_image->Visible && !$this->h_feature_image->ReadOnly && !$this->h_feature_image->Upload->KeepFile) {
				$this->h_feature_image->Upload->DbValue = $rsold['h_feature_image']; // Get original value
			...
			// h_site
			$this->h_site->SetDbValueDef($rsnew, $this->h_site->CurrentValue, NULL, $this->h_site->ReadOnly);
			...
			// create_date
			$this->create_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->create_date->CurrentValue, 1), NULL, $this->create_date->ReadOnly);

			// update_date
			$this->update_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->update_date->CurrentValue, 2), NULL, $this->update_date->ReadOnly);

			// h_online_status
			$this->h_online_status->SetDbValueDef($rsnew, ((strval($this->h_online_status->CurrentValue) == "Y") ? "Y" : "N"), NULL, $this->h_online_status->ReadOnly);

			// hotel_blocked
			$this->hotel_blocked->SetDbValueDef($rsnew, ((strval($this->hotel_blocked->CurrentValue) == "Y") ? "Y" : "N"), NULL, $this->hotel_blocked->ReadOnly);
...
<?php if (@$_POST["fa_x_h_feature_image"] == "0") { ?>
<input type="hidden" name="fa_x_h_feature_image" id= "fa_x_h_feature_image" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_h_feature_image" id= "fa_x_h_feature_image" value="1">
<?php } ?>
